...
namespace Fonty2 {
    public partial class frmMain : Form {
        ...
        // Event handlers + functions for outputting ASCII.
        private void btnPrintToOutputWindow_Click(object sender, EventArgs e) {
            if (!working) {
                updateAsciiWriter();
                Thread writer = new Thread(f => printToOutputWindow());
                writer.Start();
            }
        }
        ...
        private void printToOutputWindow() {
            working = true;
            string ascii = asciiWriter.getAscii();
            this.InvokeEx(f => outputWindow.txtOutput.Text = ascii);
            working = false;
        }
